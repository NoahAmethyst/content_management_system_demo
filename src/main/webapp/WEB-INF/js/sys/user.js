$(function () {
    ...
    var start = {
        elem: '#start',
        format: 'YYYY/MM/DD',
        min: '2018-03-16 12:12:12', //设定最小日期为当前日期
...
var vm = new Vue({
        ...
        queryUser() {
            ...
            options.columns = [
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        var actions = [];
                        //actions.push('<a style="display: '+editFlag+'" class="btn btn-success btn-xs" href="javascript:void(0)" onclick="vm.edit(\'' + row.userId + '\')"><i class="fa fa-edit"></i>编辑</a>');
                        //actions.push(' <a style="display: '+removeFlag+'" class="btn btn-danger btn-xs" href="javascript:void(0)" onclick="$.operate.remove(\'' + row.userId + '\')"><i class="fa fa-remove"></i>删除</a>');
    ...
    mounted() {
        ...
        axios.get("/sys/dictData/byType", {params:params}).then(function (res) {
            vm.userStateList = res.data.data;
            //再初始化部门和用户列表
            vm.queryDeptTree();
            vm.queryUser();
